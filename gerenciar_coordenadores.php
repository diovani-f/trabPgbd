...
// Query base para buscar coordenadores
$sql = "SELECT p.id, p.nome, p.email, 
               c.nome AS nome_curso
        FROM professor p
        LEFT JOIN curso c ON c.id_coordenador = p.id 
        WHERE 1=1";
...
<div class="container">
    ...
    <a href="cadastrar_coordenador.php" class="create">Criar Coordenador</a>
        ...
        <?php if ($resultado && $resultado->num_rows > 0): ?>
            <?php while ($row = $resultado->fetch_assoc()): ?>
                <tr>
                    ...
                    <td class="actions">
                        <button class="button edit" onclick="window.location.href='editar_coordenador.php?id=<?= $row['id']; ?>'">Editar</button>
                        <button class="button delete" onclick="window.location.href='excluir_coordenador.php?id=<?= $row['id']; ?>'">Excluir</button>
                    </td>
                </tr>
            <?php endwhile; ?>
        <?php else: ?>
            <tr>
                <td colspan="5" style="text-align:center;">Nenhum coordenador encontrado.</td>
            </tr>
        <?php endif; ?>
        </tbody>
    </table>
